package rsa

import (
	"encoding/base64"
	"fmt"
	"testing"
)

func TestRsaHandler_SetPublicKey(t *testing.T) {

	type args struct {
		pub string
	}
	tests := []struct {
		name    string
		args    args
		wantErr bool
	}{
		// TODO: Add test cases.
		{
			name: "",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := &RsaHandler{}
			err := r.SetPublicKey("-----BEGIN PUBLIC KEY-----\\niwIDAQAB\n-----END PUBLIC KEY-----")
			if err != nil {
				fmt.Println(err.Error())
				return
			}
			err = r.SetPrivateKey("-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----")
			if err != nil {
				fmt.Println(err.Error())
				return
			}
			der := []byte("实际可加密的明文长度最大是1024bits。如果小于这个长度怎么办？就需要进行数据补齐（padding），因为如果没有padding")

			de, err := r.RsaEncrypt(der)
			if err != nil {
				fmt.Println(err.Error())
				return
			}
			fmt.Println(base64.StdEncoding.EncodeToString(de))

			iu, err := r.RsaDecrypt(de)
			if err != nil {
				fmt.Println(err.Error())
				return
			}
			fmt.Println(string(iu))

			//var name = struct {
			//	Key1 string `json:"key1"`
			//	Key2 string `json:"key2"`
			//	Key3 string `json:"key3"`
			//}{Key1: "val1",
			//	Key2: "val2",
			//	Key3: "val3"}

			//der, _ := json.Marshal(name)
			//der, _ := json.MarshalIndent(name, "", "    ")
			//fmt.Println(string(der))
			//ert, err := r.RsaSignWithSha256(der)
			//if err != nil {
			//	fmt.Println(err.Error())
			//	return
			//}
			//fmt.Println(base64.StdEncoding.EncodeToString(ert))
			//ok, err := r.RsaVerySignWithSha256(der, ert)
			//if err != nil {
			//	fmt.Println(err.Error())
			//	return
			//}
			//fmt.Println(ok)
		})
	}
}
